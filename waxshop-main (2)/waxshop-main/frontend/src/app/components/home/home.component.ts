...
export class HomeComponent implements OnInit {
  ...
  private extractApiError(error: any): string {
    ...
    const angularHttpError = /^Http failure response for /;
    for (const msg of candidates) {
      if (typeof msg === 'string' && msg && !angularHttpError.test(msg)) {
        return msg;
      }
    }
    return '';
  ...
  submitLogin() {
    ...
    this.auth.login(email!, password!).pipe(
      catchError(error => {
        ...
        const apiMsg = this.extractApiError(error);
        this.loginError = apiMsg || 'Login failed. Please try again.';
